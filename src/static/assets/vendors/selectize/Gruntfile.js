...
module.exports = function (grunt) {
  ...
  grunt.registerTask("build_standalone", "", function () {
    ...
    // write output
    path = "build/js/standalone/selectize";
  ...
  grunt.initConfig({
    ...
    replace: {
      ...
      main: {
        options: {
          variables: {
            version: "<%= pkg.version %>",
            js: '<%= grunt.file.read("build/js/selectize").replace(/\\n/g, "\\n\\t") %>',
            css: '<%= grunt.file.read("dist/css/selectize.css") %>',
          },
        },
        files: [
          { src: ["src/.wrapper.js"], dest: "dist/js/selectize" },
    ...
    uglify: {
      main: {
        options: {
          banner: "/*! selectize - v<%= pkg.version %> | https://github.com/selectize/selectize.js | Apache License (v2) */\n",
          report: "gzip",
          "ascii-only": true,
          mangle: false
        },
        files: {
          ...
          "dist/js/standalone/selectize.js": [ "build/js/standalone/selectize", ],
          "dist/js/standalone/selectize.min.js": [ "build/js/standalone/selectize", ],
        },
      },
    },
    watch: {
